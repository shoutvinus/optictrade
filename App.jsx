...
// ─── STORAGE ─────────────────────────────────────────────────────
const SK = "optictrade-trades";
async function load() { try { const r = localStorage.getItem(SK); return r ? JSON.parse(r) : []; } catch { return []; } }
async function save(t) { try { localStorage.setItem(SK, JSON.stringify(t)); } catch (e) { console.error(e); } }

const IMPORTED_KEY = "optictrade-hist-imported";
...
async function importHistorical() {
  try {
    const already = localStorage.getItem(IMPORTED_KEY);
...
async function markImported() {
  try { localStorage.setItem(IMPORTED_KEY, "true"); } catch {}
}
...
export default function App() {
  ...
  const [modal, setModal] = useState(null); // {type: "close"|"edit", trade}
  const [toast, setToast] = useState(null);

  useEffect(() => {
    load().then(async (existing) => {
      const hist = await importHistorical();
      if (hist) {
        const existingIds = new Set(existing.map(t => t.id));
        const newHist = hist.filter(t => !existingIds.has(t.id));
        const merged = [...existing, ...newHist];
        await save(merged);
        await markImported();
        setTrades(merged);
      } else {
  ...
  useEffect(() => { if (loaded) save(trades); }, [trades, loaded]);

  const flash = (m) => { setToast(m); setTimeout(() => setToast(null), 2200); };

  const addTrade = (t) => { setTrades((p) => [{ ...t, id: uid(), createdAt: Date.now() }, ...p]); flash("Trade logged"); setView("dash"); };
  ...
  const updateTrade = (id, u) => { setTrades((p) => p.map((t) => t.id === id ? { ...t, ...u } : t)); setModal(null); flash("Trade updated"); };
  const deleteTrade = (id) => { setTrades((p) => p.filter((t) => t.id !== id)); setModal(null); flash("Trade removed"); };
  ...
  const resetAll = async () => {
    ...
    setTrades([]);
    try { localStorage.removeItem(SK); localStorage.removeItem(IMPORTED_KEY); } catch {}
  ...
  return (
    <div style={{ background: C.bg, minHeight: "100vh", fontFamily: "'DM Sans', sans-serif", color: C.text, paddingBottom: 80, position: "relative" }}>
      ...
      <div style={{ maxWidth: 560, margin: "0 auto", padding: "0 20px" }}>
        ...
        {view === "settings" && <Settings trades={trades} onExport={exportTrades} onImport={importTrades} onReset={resetAll} />}
      </div>
...
// ─── SETTINGS ────────────────────────────────────────────────────
function Settings({ trades, onExport, onImport, onReset }) {
  const handleImport = () => {
  ...
  return (
    <div className="fi" style={{ paddingTop: 20 }}>
      ...
      <div style={{ textAlign: "center", padding: "8px 0 20px" }}>
        ...
        <p style={{ fontSize: 11, color: C.textMuted + "80", marginTop: 4 }}>Options tracking terminal</p>
      </div>
    </div>
  );
}

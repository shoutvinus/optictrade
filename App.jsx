...
export default function App() {
  ...
  useEffect(() => {
    ...
    loadTrades(user.id).then((existing) => {
      setTrades(existing);
      setLoaded(true);
    });
  }, [user]);
  ...
  const closeTrade = (id, ep, cd, fees, closeContracts) => {
    ...
    if (toClose >= totalC) {
      const updated = { ...trade, status: "Closed", exitPrice: parseFloat(ep), closeDate: cd, fees: parseFloat(fees || 0) };
      setTrades((prev) => prev.map((t) => t.id === id ? updated : t));
      syncTrade(updated);
    } else {
      const remaining = { ...trade, contracts: totalC - toClose };
      ...
      setTrades((prev) => prev.map(t => t.id === id ? remaining : t).concat(closedPart));
      syncTrade(remaining);
      syncTrade(closedPart);
    }
    setModal(null);
    flash(closeContracts && closeContracts < totalC ? `Closed ${closeContracts} contract${closeContracts > 1 ? "s" : ""}` : "Position closed");
  };
  ...
  const expireWorthless = (id) => {
    ...
    const updated = { ...trade, status: "Closed", exitPrice: 0, closeDate: trade.expDate || today(), fees: Math.round(fee * 100) / 100, notes: (trade.notes ? trade.notes + " " : "") + "Expired worthless" };
    setTrades((p) => p.map((t) => t.id === id ? updated : t));
    syncTrade(updated);
    hapticHeavy();
  ...
  const updateTrade = (id, u) => {
    const updated = { ...trades.find(t => t.id === id), ...u };
    setTrades((p) => p.map((t) => t.id === id ? updated : t));
    syncTrade(updated);
  ...
  const importTrades = (json) => {
    try {
      ...
      if (user) bulkInsert(newTrades, user.id);
